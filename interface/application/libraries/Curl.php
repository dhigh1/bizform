...
class Curl {
	...
	protected $api_base_url='https://sartdev.in/dhurba/bizform/backend/'; // Place API base URL here
	protected $crm_api_base_url='https://sartdev.in/dhurba/bizform/backend/'; // Place API base URL here
	protected $api_key = "X-API-KEY:BIZFORM123";   // Populates extra HTTP headers also add API Key
	...
	public function crm_execute($url,$method,$params=array(),$returnMethod='',$debugMode=false)
	{
		$url=$this->crm_api_base_url.$url;
		$curl = curl_init($url);
		switch ($method) {
			case 'GET':
	...
	public function execute_old($url,$method,$params=array(),$returnMethod='',$debugMode=false)
	{
	...
	public function execute($url,$method,$params=array(),$returnMethod='',$debugMode=false)
	{
		...
		array_push($headers, "X-requested-with: ".subdomain());
		curl_setopt($curl, CURLOPT_URL, $url);
		curl_setopt($curl, CURLOPT_CUSTOMREQUEST,$method);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($curl, CURLOPT_HTTPHEADER, $headers);
