...
class Form_categories extends CI_Controller
{
		...
		if (empty($id)) {
			...
			if ($apidata['status'] == 'success') {
				if (!empty($apidata['data_list'])) {
					...
					$str = 'Service category not found...';
				}
			} else {
				$str = 'Service category not found...';
			}
	...
	public function save_data()
	{
		...
		if ($apidata['status'] == 'success' && !empty($apidata['data_list'])) {
			...
			$message=ucwords($apidata['data_list']['name'])."-".$apidata['message'];
