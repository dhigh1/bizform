...
! function($) {
    ...
    Iconpicker.VERSION = "1.10.0", Iconpicker.ICONSET_EMPTY = {
        ...
        op.selected = $.inArray(icon.replace(op.iconClassFix, ""), op.icons), -1 === op.selected && (op.selected = 0, icon = op.iconClassFix + op.icons[op.selected]), "" !== icon && op.selected >= 0 && (op.icon = icon, op.inline === !1 && (el.find("input").val(icon), el.find("i").attr("class", "").addClass(op.iconClass).addClass(icon)), icon === op.iconClassFix ? el.trigger({
            ...
            type: "change",
            icon: icon
        }), el.parent().parent().find("input[name=icon]").val(icon)), op.table.find("button." + op.selectedClass).removeClass(op.selectedClass))
    ...
    $.fn.iconpicker = function(option, params) {
        return this.each(function() {
            ...
            if (data || $this.data("bs.iconpicker", data = new Iconpicker(this, options)), "string" == typeof option) {
                ...
                "BUTTON" === $this.prop("tagName") ? ($this.empty().append("<i></i>").append('<span class="caret"></span>').addClass("iconpicker " + ("3.x" === $.fn.bsVersion() ? "" : "dropdown-toggle")), data.setIconset(op.iconset), $this.on("click", function(e) {
                ...
                })) : (op.inline = !0, data.setIconset(op.iconset), $this.empty().append(op.table).addClass("iconpicker").addClass(op.align), data.switchPage(op.icon), data.bindEvents())
            }
        })
