...
class Publishmodel extends CI_Model
{
	...
	function filter($filter_data, $item=1, $page=1,$sortby='',$orderby='',$all=true) {
        ...
        if (($item == 0) && ($page == 0)) {
            ...
            $sql = "select $this->table.*, created_users_table.login_id as created_username, updated_users_table.login_id as updated_username, 
					status_table.l_value as status_name, status_table.color_name as status_color_name from ".$this->table." 
		...
		public function check_duplicate_candidates($data){
            $arr = array(
                'phone'=>$data['phone'],
                'email'=>$data['email']
            );
            ...
            $query = $this->db->get_where('candidates', $arr);
