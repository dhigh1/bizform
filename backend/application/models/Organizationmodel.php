	 ...
	 function filter($filter_data, $item=1, $page=1,$sortby='',$orderby='',$all=true) {
        ...
        if (($item == 0) && ($page == 0)) {
            ...
            $sql = "select $this->table.*,
					$this->uploads_table.file_name as logo_name,$this->uploads_table.file_ext as logo_ext,$this->uploads_table.file_path as logo_path, favicon_table.file_name as favicon_name,
                    favicon_table.file_ext as favicon_ext, 
                    favicon_table.file_path as favicon_path 
					from $this->table					
					left join $this->uploads_table on $this->table.logo = $this->uploads_table.id
                    left join $this->uploads_table as favicon_table on $this->table.favicon = favicon_table.id
