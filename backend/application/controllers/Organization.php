...
class Organization extends REST_Controller {
    ...
    */
	...
	public function perform_update_data_field_operation($id, $data_fields) {
		...
		if (empty($data[0]['data_fields'])) {
			// Handle case when data_fields is empty
			$up_array = [];
			foreach ($data_fields as $k => $v) {
				$data_type = $v['data_type'];
				$up_array[$data_type] = $v['data_value'];
			...
			// Handle case when data_fields already exists
			foreach ($data_fields as $k => $v) {
				$data_type = $v['data_type'];
				$data_value = json_encode(array(
					"facebook" => $v['data_value']['facebook'],
					"twitter" => $v['data_value']['twitter'],
					"linkedin" => $v['data_value']['linkedin'],
					"instagram" => $v['data_value']['instagram']
				));
	
				// Prepare and execute the update query
				$data_value_escaped = $this->db->escape($data_value);
				$sql = "UPDATE organization SET data_fields = JSON_SET(data_fields, '$.$data_type', $data_value_escaped) WHERE id = $id";
				// print_R($sql);exit();
				$query = $this->db->query($sql);
				$status = 1;
			}
	
			// Remove backslashes from the data_fields column
			$r_sql = "UPDATE organization SET data_fields = REPLACE(data_fields, '\\\\', '') WHERE id = $id";
			$query = $this->db->query($r_sql);
		}
    ...
    */
...

    {
		...
		if(isset($input['tagline'])){ 
			$rules['tagline'] = ['Tagline','max_length[500]|min_length[3]'];
			$data['tagline'] = $input['tagline'];
		}
		if(isset($input['logo'])){
			$rules['logo'] = ['Logo','numeric'];
			$data['logo'] = $input['logo'];
		}
		if(isset($input['favicon'])){
			$rules['favicon'] = ['Favicon','numeric'];
			$data['favicon'] = $input['favicon'];
		}
		$message = [
