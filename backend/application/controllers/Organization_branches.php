...
class Organization_branches extends REST_Controller
{
	...
	*/
	...
	public function get_single_result($id)
	{
		...
		$filter_data[0]['type'] = 'organization_branches.id';
	...
	*/
	...
	public function index_post()
	{
		...
		$rules = [
			'name' => ['Branch name', 'required|min_length[3]|max_length[400]'],
			...
			'pincode' => ['Pincode', 'required|numeric|max_length[6]|min_length[6]|regex_match[/^[0-9]*$/]'],
			'email' => ['Email', 'required|valid_email|max_length[100]|min_length[5]'],
			'phone' => ['Phone No.', 'required|numeric|max_length[10]|min_length[10]']
		];
	...
	*/
	...
	public function index_put($id)
	{
		...
		if (!empty($input['countries_id'])) {
			$rules['countries_id'] = ['Country', 'required'];
			$data['countries_id'] = $input['countries_id'];
		}
		if (!empty($input['states_id'])) {
			$rules['states_id'] = ['State', 'required'];
			$data['states_id'] = $input['states_id'];
		}
		if (!empty($input['cities_id'])) {
			$rules['cities_id'] = ['City', 'required'];
			$data['cities_id'] = $input['cities_id'];
		}
		if (!empty($input['pincode'])) {
			$rules['pincode'] = ['PinCode', 'required|numeric|min_length[6]|max_length[6]|regex_match[/^[0-9]*$/]'];
		...
		if (!empty($input['phone'])) {
			$rules['phone'] = ['Phone No.', 'required|numeric|max_length[10]|min_length[10]'];
